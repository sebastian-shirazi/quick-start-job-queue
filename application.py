...
def get_or_add_job():
    if request.method == 'GET':
        ...
        job = q.peek()
        if job:
            return jsonify({'uuid:': job[0], 'name': job[1], 'priority': job[2], 'exec_time': job[3]})
    ...
    if request.method == 'DELETE':
        ...
        return jsonify({'Success': 'Job Removed', 'uuid:': removed_job[0], 'name': removed_job[1], 'priority': removed_job[2], 'exec_time': removed_job[3]})
